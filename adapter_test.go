...
var adapter = NewShippoAdapter()

func TestShippoAdapter_GetTrackingInfo_Transit(t *testing.T) {
	data, err := adapter.GetTrackingInfo(ShippoCarrier, ShippoTransit)

	assert.Nil(t, err)
	assert.Equal(t, "shippo", data.Carrier)
	assert.Equal(t, "SHIPPO_TRANSIT", data.TrackingNumber)
	assert.Equal(t, "San Francisco", data.AddressFrom.City)
	assert.Equal(t, "CA", data.AddressFrom.State)
	assert.Equal(t, "94103", data.AddressFrom.Zip)
	assert.Equal(t, "US", data.AddressFrom.Country)
	assert.Equal(t, "Chicago", data.AddressTo.City)
	assert.Equal(t, "IL", data.AddressTo.State)
	assert.Equal(t, "60611", data.AddressTo.Zip)
	assert.Equal(t, "US", data.AddressTo.Country)
...
func TestShippoAdapter_GetTrackingInfo_Delivered(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, "shippo", data.Carrier)
	assert.Equal(t, "SHIPPO_DELIVERED", data.TrackingNumber)
	assert.Equal(t, "San Francisco", data.AddressFrom.City)
	assert.Equal(t, "CA", data.AddressFrom.State)
	assert.Equal(t, "94103", data.AddressFrom.Zip)
	assert.Equal(t, "US", data.AddressFrom.Country)
...
func TestShippoAdapter_GetTrackingInfo_Failure(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, "shippo", data.Carrier)
	assert.Equal(t, "SHIPPO_FAILURE", data.TrackingNumber)
	assert.Equal(t, "San Francisco", data.AddressFrom.City)
	assert.Equal(t, "CA", data.AddressFrom.State)
	assert.Equal(t, "94103", data.AddressFrom.Zip)
	assert.Equal(t, "US", data.AddressFrom.Country)
